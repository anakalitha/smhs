...
type DiscountCode = "SCAN" | "PAP" | "CTG" | "LAB" | "PHARMA" | "CONSULTATION";
...
type LoadedData = {
  ...
  // ✅ from API GET (visit_discount_notes)
  discountNotes?: Partial<Record<DiscountCode, string>>;
};
...
function parseInstructionsMeta(instructions: string | null) {
  ...
  if (!metaMatch) {
    return { periodicity: "Daily", startDate: "", instructions: raw };
  }
  ...
  return { periodicity, startDate, instructions: rest };
}
...
export default function ConsultationClient({
  ...
  // Prescription notes shown in the top row
  const [rxNotes, setRxNotes] = useState("");
  const [rxItems, setRxItems] = useState<RxItem[]>([]);
  const [admit, setAdmit] = useState(false);

  // ✅ Discount notes (stored ONLY in visit_discount_notes)
  ...
  const [consultationDiscountNote, setConsultationDiscountNote] = useState("");
  ...
  function hydrate(data: LoadedData) {
    ...
    const oScan = data.orders.find((o) => o.orderType === "SCAN" && o.status !== "CANCELLED");
    const oPap = data.orders.find((o) => o.orderType === "PAP_SMEAR" && o.status !== "CANCELLED");
    const oCtg = data.orders.find((o) => o.orderType === "CTG" && o.status !== "CANCELLED");
    const oLab = data.orders.find((o) => o.orderType === "LAB" && o.status !== "CANCELLED");
    ...
    // ✅ hydrate discount notes from API (single source)
    const dn = (data as any).discountNotes || {};
    setScanDiscountNote(dn.SCAN || "");
    setPapDiscountNote(dn.PAP || "");
    setCtgDiscountNote(dn.CTG || "");
    setLabDiscountNote(dn.LAB || "");
    setPharmaDiscountNote(dn.PHARMA || "");
    setConsultationDiscountNote(dn.CONSULTATION || "");
  }
  ...
  const payload = useMemo(() => {
    return {
      ...
      prescription: {
        ...
        items: rxItems.map((it, idx) => {
          ...
          return {
            ...
            durationDays: it.durationDays ? Number(toNumStr(it.durationDays)) : null,
      ...
      // ✅ single source: visit_discount_notes
      discountNotes: {
        SCAN: scanDiscountNote,
        PAP: papDiscountNote,
        CTG: ctgDiscountNote,
        LAB: labDiscountNote,
        PHARMA: pharmaDiscountNote,
        CONSULTATION: consultationDiscountNote,
      },
    };
  }, [
    diagnosis,
  ...
  return (
    <div className={embedded ? "" : "min-h-[calc(100vh-120px)] bg-[#F2F2F2]"}>
      <div className={embedded ? "space-y-5" : "p-6 max-w-6xl mx-auto space-y-5"}>
        {/* Header */}
        ...
        <div className="rounded-2xl border bg-white shadow-sm p-5">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div>
              <div className="text-sm font-medium text-slate-700 mb-2">Diagnosis</div>
              <textarea className={textAreaClsSmall()} value={diagnosis} onChange={(e) => setDiagnosis(e.target.value)} />
            </div>

            <div>
              <div className="text-sm font-medium text-slate-700 mb-2">Investigation</div>
              <textarea className={textAreaClsSmall()} value={investigation} onChange={(e) => setInvestigation(e.target.value)} />
            </div>

            <div>
              <div className="text-sm font-medium text-slate-700 mb-2">Treatment</div>
              <textarea className={textAreaClsSmall()} value={treatment} onChange={(e) => setTreatment(e.target.value)} />
            </div>

            <div>
              <div className="text-sm font-medium text-slate-700 mb-2">Consultation Remarks</div>
              <textarea className={textAreaClsSmall()} value={remarks} onChange={(e) => setRemarks(e.target.value)} />
            </div>

            <div>
              <div className="text-sm font-medium text-slate-700 mb-2">Prescription Notes</div>
              <textarea className={textAreaClsSmall()} value={rxNotes} onChange={(e) => setRxNotes(e.target.value)} />
            </div>
          </div>
        </div>

        {/* Consultation Discount Note */}
        <div className="rounded-2xl border bg-white shadow-sm p-5">
          ...
          <textarea
            ...
            placeholder="Consultation discount note (optional)"
            value={consultationDiscountNote}
            onChange={(e) => setConsultationDiscountNote(e.target.value)}
          />
        </div>
        ...
        <div className="rounded-2xl border bg-white shadow-sm p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="rounded-xl border bg-slate-50 p-3 space-y-2">
              <Toggle label="Scan" checked={scanNeeded} onChange={setScanNeeded} />
              <textarea
            ...
            <div className="rounded-xl border bg-slate-50 p-3 space-y-2">
              <Toggle label="PAP Smear" checked={papNeeded} onChange={setPapNeeded} />
              <textarea
                className={textAreaClsMedium()}
              ...
              <textarea
                className={textAreaClsSmall()}
                placeholder="PAP Discount/Waiver note (optional)"
            ...
            <div className="rounded-xl border bg-slate-50 p-3 space-y-2">
              <Toggle label="Lab Tests" checked={labNeeded} onChange={setLabNeeded} />
              <textarea
                className={textAreaClsMedium()}
        ...
        <div className="rounded-2xl border bg-white shadow-sm p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              onClick={() => setRxItems((prev) => [...prev, blankRxItem(prev.length)])}
            >
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-slate-600">
                <tr className="border-b">
                  <th className="px-2 py-2 text-left font-medium min-w-[220px]">Medicine</th>
                  <th className="px-2 py-2 text-center font-medium min-w-[170px]">Dosage (M / A / N)</th>
                  <th className="px-2 py-2 text-center font-medium min-w-[110px]">Before food</th>
                  <th className="px-2 py-2 text-left font-medium min-w-[110px]">Duration</th>
                  <th className="px-2 py-2 text-left font-medium min-w-[140px]">Periodicity</th>
                  <th className="px-2 py-2 text-left font-medium min-w-[150px]">Start Date</th>
                  <th className="px-2 py-2 text-left font-medium min-w-[240px]">Instructions</th>
                  ...
                  <tr key={idx} className="border-b last:border-b-0 align-top">
                    <td className="px-2 py-2">
                      <MedicineComboBox
                        ...
                        onChange={(v) =>
                          setRxItems((prev) => prev.map((x, i) => (i === idx ? { ...x, medicineName: v } : x)))
                        }
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setRxItems((prev) => prev.map((x, i) => (i === idx ? { ...x, morningDose: v } : x)));
                          }}
                        />
                        <input
                          className={inputCls()}
                          ...
                          onChange={(e) => {
                            const v = toNumStr(e.target.value);
                            setRxItems((prev) => prev.map((x, i) => (i === idx ? { ...x, afternoonDose: v } : x)));
                          }}
                        />
                        <input
                          className={inputCls()}
                          ...
                          onChange={(e) => {
                            const v = toNumStr(e.target.value);
                            setRxItems((prev) => prev.map((x, i) => (i === idx ? { ...x, nightDose: v } : x)));
                          }}
                        />
                      </div>
                    </td>
                    ...
                    <td className="px-2 py-2 text-center">
                      <input
                        ...
                        onChange={(e) =>
                          setRxItems((prev) => prev.map((x, i) => (i === idx ? { ...x, beforeFood: e.target.checked } : x)))
                        }
                      />
                    </td>
                    ...
                    <td className="px-2 py-2">
                      <input
                        ...
                        onChange={(e) => {
                          const v = toNumStr(e.target.value);
                          setRxItems((prev) => prev.map((x, i) => (i === idx ? { ...x, durationDays: v } : x)));
                        }}
                      />
                    </td>
                    ...
                    <td className="px-2 py-2">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setRxItems((prev) => prev.map((x, i) => (i === idx ? { ...x, periodicity: v } : x)));
                        }}
                      />
                    </td>
                    ...
                    <td className="px-2 py-2">
                      <input
                        ...
                        onChange={(e) => {
                          const v = e.target.value;
                          setRxItems((prev) => prev.map((x, i) => (i === idx ? { ...x, startDate: v } : x)));
                        }}
                      />
                    </td>
                    ...
                    <td className="px-2 py-2">
                      <textarea
                        ...
                        onChange={(e) => {
                          const v = e.target.value;
                          setRxItems((prev) => prev.map((x, i) => (i === idx ? { ...x, instructions: v } : x)));
                        }}
                    ...
                    <td className="px-2 py-2 text-right">
                      <button
                        ...
                        title={rxItems.length === 1 ? "At least one row required" : "Remove"}
                      >
