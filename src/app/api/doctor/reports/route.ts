...
export async function GET(req: Request) {
  ...
  // NOTE:
  // - visit_orders has columns: order_type, status, notes
  ...
  const [rows] = await db.execute<RowDataPacket[]>(
    ...
    SELECT
      ...
      -- Latest notes per order type
      MAX(CASE WHEN o.order_type='SCAN' AND o.status <> 'CANCELLED' THEN o.notes END) AS scanDetails,
      MAX(CASE WHEN o.order_type='PAP_SMEAR' AND o.status <> 'CANCELLED' THEN o.notes END) AS papSmearDetails,
      MAX(CASE WHEN o.order_type='CTG' AND o.status <> 'CANCELLED' THEN o.notes END) AS ctgDetails,

      -- Treatment: comma separated medicines (distinct)
      (
