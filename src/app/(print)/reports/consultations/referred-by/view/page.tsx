...
export default async function ReferredByReportView({
  searchParams,
}: {
  searchParams: Promise<{ from?: string; to?: string; referralId?: string }>;
}) {
  ...
  const { from, to, referralId } = await searchParams;
  ...
  const [rows] = await db.execute<Row[]>(
    ...
    { org: orgId, branch: branchId, from, to, refId: referralId }
  );
  ...
  return (
    <div className="report-wrap max-w-6xl mx-auto">
      <ReportToolbar
        canExport={canExport}
        exportCsvUrl={`/api/reports/consultations/referred-by/export?from=${from}&to=${to}&referralId=${encodeURIComponent(
          referralId
        )}`} // if you have CSV
        exportXlsxUrl={`/api/reports/consultations/referred-by/export-xlsx?from=${from}&to=${to}&referralId=${encodeURIComponent(
          referralId
        )}`}
      />
      ...
      <div className="p-10">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <div className="text-xl font-bold">Referred By Report</div>
            <div className="text-sm text-gray-600">
              Referred By: <span className="font-medium">{referralName}</span>
            </div>
            <div className="text-sm text-gray-600">
